...
test("send clears the composer before the round trip and restores a rejected draft (D287)", () => {
  ...
  const clearAt = submit.indexOf("draft.clearDraftForKey(submittedDraftKey, submittedDraft);\n    const accepted = steering");
  ...
  assert.doesNotMatch(submit, /if \(accepted\) draft\.clearDraftForKey\(submittedDraftKey, submittedDraft\);\s*\};/);
...
test("mode slash prefixes send the trailing prompt and retain failed drafts", () => {
  ...
  assert.match(
    submit,
    /if \(isModeCommand && commandBody\)[\s\S]*?await runPaletteCommand\(command\.id\);[\s\S]*?const accepted = await sendPrompt\([\s\S]*?draft\.draftSnapshot\(visibleCommandBody\)[\s\S]*?if \(accepted\) draft\.clearDraftForKey\(submittedDraftKey, submittedDraft\);/,
  );
  assert.match(
    submit,
