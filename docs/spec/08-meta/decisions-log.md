...
## 2026-09-20 — A copied formula is its TeX source (D618, issue #414)
...
- Splitting a message into independently parsed blocks, which predates D618 and
  which D618 moved onto the rendering grammar, is only sound for a slice that
...
- D618 adds no surface, no store state, and no action-row item: it is the OS
