...
- Preview mode unmounts MainChat and lets the work panel fill the client area
  ...
  panel header reserves the 88px windowed (8px fullscreen) traffic-light inset,
  the preview action lane, and an 8px gap before its first tab. That 88px is the
  shared `--ds-window-lead-inset` token: the traffic-light cluster's 76px right
  edge (native geometry from `@pi-desktop/shared`) plus a 12px gap.
