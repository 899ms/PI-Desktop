...
- **Steps**: Send `https://en.wikipedia.org/wiki/React_(software)` in a user message and click its link. Repeat with the URL wrapped in prose parentheses.
- **Expected**: The complete URL, including `(software)`, opens the React software article. The outer prose closing parenthesis is not part of the link. Nested parentheses, query/fragment parentheses and percent-encoded parentheses remain intact.
