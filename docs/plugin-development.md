...
`keyboard.globalShortcut` is implemented. `pi.audio.*` and `pi.net.websocket.*`
are declared and gated but not implemented yet: every call fails closed with
`UNSUPPORTED` until the host services land.
