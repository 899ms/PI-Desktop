# ADR 0283: `RACP-WS` transport in `packages/racp`

- Status: Accepted for implementation
- Date: 2026-09-18
- Decision: D446
- Related: ADR 0205 (D373 / D374 / D375), ADR 0282 (D445),
...
- `pi-host` composes `RacpServer` + `bindRacpWebSocket` over the module and
  `RuntimeService` (ADR 0282); the desktop adapter composes `RacpClient` +
  `wsClientTransport`.
