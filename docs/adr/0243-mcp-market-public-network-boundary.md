...
1. Shared renderer and Main URL validation accepts credentials-free HTTPS only.
   ...
   forms.
2. Electron Main resolves every hostname immediately before a request, rejects
   any non-public result, and pins the selected public address to the actual
   HTTPS socket while retaining the original host as TLS SNI and HTTP Host.
   Redirects are manual, HTTPS-only, and revalidated and repinned at every hop,
   with a maximum of five redirects.
3. Market source responses are bounded to 4 MiB, DNS and HTTP requests share an
   eight-second deadline, at most 16 sources are processed per call, and the
   in-memory catalog/search caches are bounded.
4. Registry npm packages are mapped as `identifier@version` and PyPI packages
...
5. Market HTTP catalog entries are restricted to public HTTPS endpoints. Manual
