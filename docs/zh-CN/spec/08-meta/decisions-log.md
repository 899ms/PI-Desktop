...
- 浮层提供三个动作：「添加到对话」（`chat.addToChat`）按 D398 的引用契约把摘录写入当前会话的 composer 草稿；
...
- 2000 字符上限、`> ` 引用块、`chat.quoteSource` 归属、聚焦行为，以及不发送、不建会话、不写转录的边界均未改变。
  ...
  存储 schema、IPC 或权限。见 ADR 0223 与 E2E-249、E2E-255、E2E-256。

## 2026-09-11 —— 响应批注作为提示词附件（D400）

- 在助手回合中选中文字并选择「添加到对话」，会为该回合附加一条**带编号的批注**：id、锚定的回合、摘录（Markdown，
  上限 2000 字符）、空的评论字段，以及作为编号的捕获顺序。批注属于渲染器侧的会话状态；重复批注同一摘录不生效，
  既不持久化，也不单独发送给宿主。
- 被批注的回合会在该段落末尾绘制行内编号标记（把 `:codex-annotation{index="N"}` 标记注入该回合源文本，由 remark
  阶段渲染为编号元素，提示内容为摘录）。composer 显示一个带数量的附件芯片，其提示列出 `N. 摘录`，并提供一个一键清除
  控件。芯片不会进入可编辑文本，因此 D209 的智能停止与 D301 的草稿保留均不受影响。
...
- 引用用户消息以及侧边聊天的 Add to main chat 仍沿用 D398 的草稿引用。不改协议、存储 schema、IPC 通道或权限。
  见 ADR 0224 与 E2E-257、E2E-258。
