...
| D398 | 消息引用与渲染器拥有的侧边聊天 | **修订 D209 / D301：每条用户消息和每个助手回合的悬停操作行新增 Quote 操作，与 Copy、Edit、Delete、Fork、Retry 并列。它把以 `> ` 开头的 Markdown 引用块加一行 `chat.quoteSource` 归属写入当前会话的 composer 草稿并聚焦输入框；选区位于被点击消息行内时使用该选区，否则使用消息自身文本；摘录上限 2000 字符并带省略号；绝不发送，也不新增任何芯片类型。Open side chat 通过 `session.fork` 以锚点消息分叉且不激活子会话，把子会话注册为父会话的渲染器侧边聊天，并在现有停靠面板中打开一个 `sidechat:<childSessionId>` 标签，用同一条事件流经后台转录 reducer 实时流式渲染，提供 Add to main chat、Open as a conversation、紧凑 Send/Stop 输入和现有权限卡。关闭标签或以会话方式打开子会话会移除注册；持久子会话仍是普通会话。不改协议、schema、IPC 或权限。见 ADR 0223 与 E2E-249 至 E2E-254。** | 用户需要复用之前某条消息或答案的原文，并在不替换当前可见主对话的前提下追问一个旁支问题，而现有 fork 路径总会激活子会话。 *（由 D399 修订：摘录改为从渲染后的 DOM 还原为 Markdown，且消息行内的选区可从选区末行的悬浮按钮直接引用。）* |
| D399 | 跟随选区的引用按钮 | **修订 D398：消息行内存在非空文本选区时，在选区末行悬浮一个 `chat.quoteSelection` 按钮，限制在视口内，并在滚动、窗口尺寸变化、选区折叠、点击引用以及只读投影中隐藏；点击后只写入 composer 草稿并聚焦输入框，绝不发送。摘录改为从渲染后的 DOM 还原为 Markdown——公式取 KaTeX 的 `application/x-tex` 注解还原为 `$…$` / `$$…$$`，代码块还原为带语言的围栏，表格每行还原为一条 `a | b` 行——消息末尾的 Quote 操作复用同一条还原路径。仅渲染器变更；不改协议、存储、IPC、权限或主机。见 ADR 0223 与 E2E-249、E2E-255。** | Quote 操作位于消息末尾，引用正在阅读的句子或公式必须离开该段落；而直接使用 DOM 选区会把每个 KaTeX 公式重复两遍、把表格压平。 |
...
## 2026-09-11 —— 消息引用与渲染器拥有的侧边聊天（D398）
...
- 关闭标签或以会话方式打开子会话会移除注册及其转录投影；持久子会话仍是侧边栏、
  ...
  见 ADR 0223 与 E2E-249 至 E2E-254。

## 2026-09-11 —— 跟随选区的引用按钮（D399）

- 引用不再必须回到答案末尾。消息行内存在非空文本选区时，在选区末行悬浮一个
  `chat.quoteSelection` 按钮：坐标基于视口并限制在视口内（默认位于选区下方，空间不足时
  翻到上方），portal 到转录之上，并在滚动对话、改变窗口尺寸、折叠选区或点击引用后隐藏。
  按钮按下时阻止默认行为，避免按下动作先折叠选区。
- 摘录改为从渲染后的 DOM 还原，而不是 `Selection.toString()`。与公式相交的选区会扩展到整个
  公式，并取 KaTeX 的 `application/x-tex` 注解还原为 `$…$` 或 `$$…$$`；代码块还原为带语言的
  围栏，围栏长度超过片段内最长的反引号串；表格每行还原为一条 `a | b` 行；行内代码保留反引号，
  文件引用芯片保留其代码文本。消息末尾的 Quote 操作与悬浮按钮共用这一条还原路径。
- 2000 字符上限、`> ` 引用块、`chat.quoteSource` 归属、聚焦行为，以及不发送、不建会话、不写转录的
  边界均未改变，只读投影规则同样适用：停靠的侧边聊天不渲染引用按钮。仅渲染器变更；不改协议、
  存储 schema、IPC 或权限。见 ADR 0223 与 E2E-249、E2E-255。
