...
| D399 | 选区的引用浮层 | **修订 D398：当消息行内存在非空文本选区时，在其**上方**悬浮一个浮层——水平居中于选区，限制在裁剪祖先（转录滚动容器）的矩形内且不越过停靠输入区的顶边，按应用体范围 portal，并在对话滚动时跟随选区移动。浮层提供「添加到对话」（`chat.addToChat`，按 D398 的引用契约把摘录写入当前会话的 composer 草稿）、「在侧边聊天中提问」（`chat.askInSideChat`，以该行为锚点分叉，并把摘录作为子会话的提问发送）、以及复制（复用 `chat.copy`）；每个动作都会先清除原生选区，跨消息行的拖选不显示浮层，只读投影不渲染浮层。摘录改为从渲染后的 DOM 还原为 Markdown——公式取 KaTeX 的 `application/x-tex` 注解还原为 `$…$` / `$$…$$`，代码块还原为带语言的围栏，表格每行还原为一条 `a | b` 行——消息末尾的 Quote 操作复用同一条还原路径。除 composer 的 `data-composer-dock` 钩子外，不改协议、存储、IPC、权限或主机。见 ADR 0223 与 E2E-249、E2E-255、E2E-256。** | Quote 操作位于消息末尾，引用正在阅读的句子或公式必须离开该段落；ChatGPT 桌面版为此提供选区浮层，本决策与之对齐。 |
