...
用户自己填写的源地址改由 ADR 0304 判定：可以是回环或局域网目录服务，明文 `http` 需要先在设置里开启 `networkPolicy.allowInsecureUserEndpoints`。来自目录正文内部的文档 URL、以及每一次重定向目标，仍然沿用上面的公网策略。
...
用户自己填写的市场源地址同样由 ADR 0304 判定：可以是回环或局域网端点，明文 `http` 需要开启 `networkPolicy.allowInsecureUserEndpoints`。源返回的任何内容——registry 记录、目录正文、重定向目标——仍然沿用上面的公网策略。
