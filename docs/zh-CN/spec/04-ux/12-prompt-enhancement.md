...
处理器为单次请求设置 60 秒上限。仅提供商重试预算就可能耗掉约一分钟，而渲染层没有
取消入口，因此超时后该操作以 `TIMEOUT` 失败，并给出写明预算与可改设置的消息。它不会
静默改用会话模型重试：模型是用户指定的，隐藏的第二次尝试会让等待时间翻倍。
...
当设置了 `promptEnhancementProviderId` 时，主进程优先使用该指定项；若无法解析，则记录
警告并回退到输入框当前模型：过期的指定项只是无法满足的偏好，绝不会成为使该操作不可用
的失败。
