...
- 带有图标和简洁平行标签的紧凑导航目录，按以下顺序：
  ...
  11. **远程主机** — Lucide `Globe`（SSH 引导与配对清单；仅开发者模式）
...
- 目录仍是保持相同顺序的可搜索扁平列表。为便于扫描，目的地分为四个带标题
  ...
  MCP、子智能体）、“工作区”（导入、项目）和“系统”（远程主机、信息）。标题使用柔和
...
- **网络**卡：
  ...
  - **自填端点的明文 HTTP**：一个复选框，持久化为 `AppSettings.networkPolicy.allowInsecureUserEndpoints`（默认关闭）。开启后，你自己填写的端点（模型服务、MCP 服务器、市场源）可以用 `http` 访问；风险提示说明该连接不加密、同一网段的人可以读到该端点接受的一切凭据。https 访问局域网主机不需要该开关（ADR 0304）。
...
2. 导轨顶部显示搜索药丸，底部固定返回应用程序操作并与主侧边栏底部图标行同一条线，
   ...
   子智能体、导入、项目和信息（开发者模式开启时，远程主机位于项目与信息之间），
