...
- **步骤**：在用户消息中发送 `https://en.wikipedia.org/wiki/React_(software)` 并点击链接，再验证正文用圆括号包裹该网址的情况。
- **预期**：打开包含 `(software)` 的完整网址，进入 React 软件词条；正文外层的右括号不属于链接。嵌套圆括号、查询和片段内的圆括号、百分号编码的圆括号均保持完整。
