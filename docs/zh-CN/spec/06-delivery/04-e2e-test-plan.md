...
#### E2E-NAV-utility-buttons-return-to-source：切换插件页并从设置返回

- **先决条件**：隔离配置中有具名会话及可见消息；不需要模型凭据或外部插件
  市场连接。
- **步骤**：1) 选择会话，从页脚进入插件页，再次点击同一按钮。2) 重开插件
  页，在已安装列表输入搜索文本，返回后再次打开。3) 从插件进入设置，切换
  设置标签，再点击返回应用。4) 用导航测试分别从 `pulls`、`scheduled`
  进入和退出工具页面。5) 在 store 测试中，于工具页打开期间删除当前会话
  或改选其他会话，再返回。6) 测试后退/前进及不记录历史的直接设置导航。
- **预期**：插件按钮切换按下/激活状态并恢复来源页面。设置通过已有返回
  控件退出，全窗布局不增加页脚。跨插件和设置导航保留最近的非工具页面。
  当前会话和工作区保持现状，不重新加载会话或恢复失效会话 ID。插件浏览
  标签、搜索文本和分类筛选跨页面卸载保留；对话框与监听正常释放，不在
  隐藏页继续运行。没有历史来源时回退到聊天页。
- **链接规格**：`04-ux/08-component-spec.md` §3、`04-ux/06-settings-ia.md`
- **验收**：C（会话导航）、G（插件浏览）、Quality
- **里程碑**：M6+
- **状态**：store/浏览状态回归已验证。使用合成会话的真实 macOS UI 验收
  确认：再次点击插件按钮恢复相同的两条消息和未发送草稿，并取消按钮激活
  状态；重开保留已安装列表的搜索筛选。插件 → 设置 → 返回应用同样恢复
  原会话和草稿。非聊天路由及会话删除/改选通过 store 测试，未做原生 UI
  测试。不需要外部插件市场或真实模型。这些均为合并前结果；主线合并后
  E2E 尚未运行。
