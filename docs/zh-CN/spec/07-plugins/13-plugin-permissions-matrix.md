...
| `net.websocket` | 高 | `pi.net.websocket.connect` / `send` / `close`（仅 SDK 类型；本条分支没有运行时） | 默认拒绝 | 与 `net.fetch` 一样被限制在 `manifest.net.domains` 之内；宿主服务落地之前每次调用都以 `UNSUPPORTED` 失败即关闭 |
