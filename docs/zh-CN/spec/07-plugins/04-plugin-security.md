...
- **远程 MCP 端点。** 同样听这份列表，而不是只看它自己的权限。HTTP 端点可以
  ...
  检查白名单。
- **`pi.net.websocket`（已规划，本条分支尚未实现）。** 目前还不存在可检查
  的套接字。规定的行为是听同一份列表：目标主机不在 `manifest.net.domains`
  里的 `ws://` 或 `wss://` 会在出网卡点处被拒绝，插件卸载或被禁用时宿主
  会关闭它仍持有的每个套接字
...
`audio.capture.background`、`audio.playback.background` 和 `net.websocket`
在这条分支里有权限和 SDK 类型，但没有宿主实现，所以每次 `pi.audio.*` /
`pi.net.websocket.*` 调用都以 `UNSUPPORTED` 失败即关闭，目前没有任何东西
能到达设备或套接字。
